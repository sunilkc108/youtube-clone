...
import { otherSuggestedVideos } from "../../../data/otherSuggestedVideos"
import type { VideoItem } from "../../../data/otherSuggestedVideos"
...
const TaggedVideos = ({ activeTag }: TaggedVideosProps) => {
  ...
  const filteredVideos: VideoItem[] =
    activeTag === "All" ? otherSuggestedVideos : otherSuggestedVideos.filter((v) => v.tags.includes(activeTag))
...
width = "168"
  ...
  <div className="flex flex-col gap-0.5 min-w-0" >
    <h2 className="text-sm font-medium line-clamp-2 leading-tight" > { vid.title } </h2>
      < p className = "text-xs text-gray-400" > { vid.channel } </p>
        < p className = "text-xs text-gray-400" > 11M views • 1 month ago </p>
          </div>
          </div>
