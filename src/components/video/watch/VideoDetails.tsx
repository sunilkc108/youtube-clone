        ...
        <div className="h-10 w-10 bg-red-300 rounded-full" > </div>
          ...
          <h1 className="font-semibold text-l" > { channel } </h1>
            < p className = "text-gray-500 text-xs" > 2.2M subscribers </p>
              ...
              < button className = {`py-[6px] px-3  rounded-full font-semibold text-sm ml-5 cursor-pointer flex items-center gap-2 ${animate ? 'animate-bounce' : 'scale-100'} ${subscribe ? 'bg-gray-100' : 'text-white bg-[#0f0f0f]'}`
}
onClick = { handleSubs }
  >

  { subscribe && <HiOutlineBell size={ 20 } className = "transition-transform duration-300" />}
    ...
    <div className="flex items-center" >
      <button className="bg-[#f2f2f2] flex items-center justify-center gap-1 py-[6px] px-3 rounded-l-2xl hover:bg-gray-300 cursor-pointer" >
        <HiOutlineThumbUp size={ 18 } />
          < span className = "font-medium text-sm" > 398K </span>
            </button>
...
{ views } views { published } { " " }
  ...
  < p className = "text-sm" >
    Lorem ipsum dolor sit amet consectetur adipisicing elit.Molestiae
            dicta iste debitis, nemo numquam ut nulla animi et ipsum
  </p>
  </div>
  < div className = "bg-[#f2f2f2] w-full rounded-xl p-4" >
